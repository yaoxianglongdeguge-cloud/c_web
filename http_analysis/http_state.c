...
char* http_state_judge(http_ed_store* hs,int* error)//errpr为-2时代表着没有http请求，要马上断开连接，-1时代表不完整，需要accept,0时代表错误，1时代表对
{
    *error=0;
    ...
    if(hs->httpstate->h_method==0)
    {
        ...
        char* method=strnstr_match(hs->begin,"POST ",8);
        char* method=strnstr_match(hs->begin,"PUT ",8);
        char* method=strnstr_match(hs->begin,"DELETE ",8);
        char* method=strnstr_match(hs->begin,"HEAD ",8);
        char* method=strnstr_match(hs->begin,"OPTIONS ",8);
        char* method=strnstr_match(hs->begin,"PATCH ",8);   
        if(method==NULL)
        {
            *error=-2;
            return NULL;
